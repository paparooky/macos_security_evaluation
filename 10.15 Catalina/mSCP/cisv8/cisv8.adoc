...
:show_STIG!:
:show_cis:
:version: Catalina Guidance, Revision 6 (2022-03-16)
:os: 10.15
:proj-title: macOS 10.15: Security Configuration - CISV8
:document-title: macOS 10.15
:document-subtitle:  Security Configuration - CISV8
:document-subtitle2:
ifdef::backend-pdf[]
= {document-title}: {document-subtitle}
{document-subtitle2}
Catalina Guidance, Revision 6 (2022-03-16)
...
[source,bash,options="nowrap"]
----
/usr/bin/profiles show -output stdout-xml | /usr/bin/xmllint --xpath 'string(//*[contains(text(), "DisabledPreferencePanes")]/following-sibling::*[1])' - | /usr/bin/grep -c com.apple.preferences.AppleIDPrefPane
...
[source,bash,options="nowrap"]
----
/usr/bin/profiles -P -o stdout | /usr/bin/grep -c 'allowCloudCalendar = 0' | /usr/bin/awk '{ if ($1 >= 1) {print "1"} else {print "0"}}'
----
...
* Whitelisting Approved Kernel Extensions
...
|
|===
=== Disable Apple Filing Protocol Sharing
...
|
|===
=== Disable Guest Access to Shared Apple File Protocol Folders
...
[source,bash,options="nowrap"]
----
/usr/bin/sudo -u "$CURRENT_USER" /usr/bin/defaults read com.apple.amp.mediasharingd | /usr/bin/grep -Ec '("public-sharing-enabled" = 0;|"home-sharing-enabled" = 0;)'
...
The screen saver timeout _MUST_ be set to 1200 minutes. 
